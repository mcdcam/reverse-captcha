...
const CaptchaChallenge = (props) => {
    return (
        <div>
            <input value={props.inputvalue} onChange={evt => props.updateInputValue(evt)} onKeyPress={evt => props.submitOnEnter(evt)} type={'text'} className='challenge-input'></input>
        </div>
    );
}
        ...
        return (
            <div className='captcha'>
                <div style={{opacity: props.opacity}} className='popup-wrapper'>
                    <div className='popup' id='popup' data-show={props.showPopper} ref={setPopperElement} style={popperObject.styles.popper} {...popperObject.attributes.popper} >
                        ...
                        <div className='popup-footer'>
                            ...
                            <button type='button' onClick={props.verifyInputValue} id='submit-button' className='submit-button'>VERIFY</button>
                        </div>
                </div>
                    </div>
...
class Captcha extends react.Component {
    ...
    verifyInputValue = () => {
        if (this.state.headercolor === 'popup-header-red') { // this is horrible but i'm lazy
            return;
        }
        if (this.state.inputvalue !== '' && String(this.state.inputvalue) === String(this.state.challenge[3])) {
            this.hidePopper();
            this.setState({
                checkhidden: '',
                buttonhidden: 'hidden'
